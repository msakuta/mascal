...
use mascal_lsp_server::semantic_analyze::{analyze_program, IdentType, Semantic};
use mascal_lsp_server::semantic_token::LEGEND_TYPE;
use mascal_lsp_server::span::Span;
use ropey::Rope;
...
impl LanguageServer for Backend {
    ...
    async fn inlay_hint(
        ...
        let inlay_hint_list = hashmap
            ...
            .filter_map(|(start_pos, end_pos, str)| {
                ...
                // let end = k.end;
                // let str = match v {
                //     mascal_lsp_server::nrs_lang::Value::Null => "null".to_string(),
                //     mascal_lsp_server::nrs_lang::Value::Bool(_) => "bool".to_string(),
                //     mascal_lsp_server::nrs_lang::Value::Num(_) => "number".to_string(),
                //     mascal_lsp_server::nrs_lang::Value::Str(_) => "string".to_string(),
                // };
                // let start_position = offset_to_position(item.0, document)?;
                // let end_position = offset_to_position(end, &document)?;
    ...
    //             match item {
    //                 mascal_lsp_server::completion::ImCompleteCompletionItem::Variable(var) => {
    ...
    //                     });
    //                 }
    //                 mascal_lsp_server::completion::ImCompleteCompletionItem::Function(
    //                     name,
    //                     args,
    //                 ) => {
    //                     ret.push(CompletionItem {
...
impl Backend {
    async fn on_change<'a>(&self, params: TextDocumentItem<'a>) {
        dbg!(&params.version);
        // let rope = ropey::Rope::from_str(dbg!(params.text));
        // self.document_map
