...
use crate::{
    bytecode::{Bytecode, FnBytecode, FnProto, Instruction, OpCode},
    interpreter::{
...
fn interpret_fn(
    ...
    while call_stack.clast()?.has_next_inst() {
        ...
        let op: OpCode = ci.fun.instructions[ip].op;
        ci.ip += 1;
        ...
        match op.arity() {
            ...
            1 => (arg0, arg1) = (Some(inst.arg0), None),
            2 => (arg0, arg1) = (Some(inst.arg0), Some(inst.arg1)),
            _ => unreachable!(),
        }
        ...
        match op {
            ...
            OpCode::Move => {
                ...
                let val = vm.get(arg0.unwrap() as usize).clone();
                vm.set(arg1.unwrap() as usize, val);
            }
            OpCode::Incr => {
                let val = vm.get_mut(arg0.unwrap() as usize);
            ...
            OpCode::Add => {
                ...
                vm.set(arg0.unwrap() as usize, result);
            }
            OpCode::Sub => {
            ...
            OpCode::And => {
                let result = truthy(&vm.get(arg0.unwrap() as usize))
                    && truthy(&vm.get(arg1.unwrap() as usize));
                vm.set(arg0.unwrap() as usize, Value::I32(result as i32));
            }
            OpCode::Or => {
                let result = truthy(&vm.get(arg0.unwrap() as usize))
                    || truthy(&vm.get(arg1.unwrap() as usize));
            ...
            OpCode::BitNot => {
                let val = vm.get(arg0.unwrap() as usize);
            ...
            OpCode::Get => {
                let target_collection = &vm.get(arg0.unwrap() as usize);
                let target_index = &vm.get(arg1.unwrap() as usize);
                ...
                vm.set(arg1.unwrap() as usize, new_val);
            }
            OpCode::Set => {
                let target_collection = &vm.get(arg0.unwrap() as usize);
                let value = vm.get(arg1.unwrap() as usize);
                let index = vm.set_register;
                target_collection.array_assign(index, value.clone())?;
            }
            OpCode::SetReg => {
                vm.set_register = coerce_i64(vm.get(arg0.unwrap() as usize as usize))? as usize;
            }
            OpCode::Lt => {
                ...
                vm.set(arg0.unwrap() as usize, Value::I64(result as i64));
            }
            OpCode::Gt => {
                let result = compare_op(
                    &vm.get(arg0.unwrap() as usize),
            ...
            OpCode::Jt => {
                if truthy(&vm.get(arg0.unwrap() as usize)) {
            ...
            OpCode::Call => {
                let arg_name = vm.get(arg1.unwrap() as usize);
                ...
                if let Some((_, fun)) = fun {
                    match fun {
                        ...
                        FnProto::Native(nat) => {
                            ...
                            vm.set(arg1.unwrap() as usize, ret?);
                        }
                    }
                } else {
            ...
            OpCode::Ret => {
                ...
                if let Some(prev_ci) = call_stack.pop() {
                    if call_stack.is_empty() {
                        return Ok(vm.get(arg1.unwrap() as usize).clone());
                    } else {
            ...
            OpCode::Cast => {
                let target_var = &vm.get(arg0.unwrap() as usize);
                let target_type = coerce_i64(vm.get(arg1.unwrap() as usize))
                ...
                vm.set(arg0.unwrap() as usize, new_val);
            }
            OpCode::If => {
                if !truthy(&vm.get(arg0.unwrap() as usize)) {
...
pub(crate) fn find_end(
    ...
    for ip2 in ip..instructions.len() {
        let op: OpCode = instructions[ip2].op;
        match op {
            OpCode::End => {
                blk_count -= 1;
                if blk_count == 0 {
